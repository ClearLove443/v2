import{_ as n,V as s,W as a,a0 as e}from"./framework-8e76daeb.js";const t={},p=e(`<h2 id="函数式接口" tabindex="-1"><a class="header-anchor" href="#函数式接口" aria-hidden="true">#</a> 函数式接口</h2><h3 id="匿名函数和闭包-的体现" tabindex="-1"><a class="header-anchor" href="#匿名函数和闭包-的体现" aria-hidden="true">#</a> 匿名函数和闭包 的体现</h3><ul><li><p>java 中无法把一个函数作为参数进行传递，返回结果也无法是一个函数</p></li><li><p>参数和返回只能是基本变量和实例化的对象</p></li></ul><h3 id="函数式接口-1" tabindex="-1"><a class="header-anchor" href="#函数式接口-1" aria-hidden="true">#</a> 函数式接口：</h3><ol><li>如果一个接口只有一个抽象方法，则是函数式接口。</li><li>如果某个接口上声明了 FunctionalInterface 注解，则是函数式接口。</li><li>如果接口只有一个抽象方法，但是没有声明 FunctionalInterface,依旧是。</li><li>函数式接口必须要有一个抽象方法， 如果接种中定义的抽象方法和 Object 类中的方法名相同，则不能认为是一个 函数式接口，因为对象或多或少都会继承 Object。接口的抽象方法不会加 1。</li><li>lambda 表达式必须是方法引用或构造函数引用。</li><li>高阶函数：如果一个函数接收一个函数作为参数，或者返回一个函数作为返回值，那么该函数就叫做高阶函数</li><li>接口中也可以定义方法，方法用 default 修饰。默认方法，保证和老版本代码兼容。实现类默认拥有 接口中的默认方法。</li><li>java 中 lambda 是一个对象，其它语言中是函数,称为函数式接口.</li><li>函数式接口必须有一个上下文。 MyInterface my = () -&gt; System.out.println(&quot;123&quot;); 必须这样（）-&gt;{} 才能存在，MyInterface 就是上下文，用来提供类型推断</li></ol><div class="language-java line-numbers-mode" data-ext="java"><pre class="language-java"><code><span class="token comment">//函数时接口</span>
